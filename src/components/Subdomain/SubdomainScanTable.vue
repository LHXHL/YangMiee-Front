<template>
  <div class="space-y-6">
    ...
    <div v-if="subdomainScanResults?.length > 0" class="flex flex-col gap-6">
          ...
          <table class="w-full">
            ...
            <tbody>
              <tr
                ...
                :class="index % 2 === 0 ? 'bg-light-bg/50' : ''"
              >
                <td class="py-3 px-4">
                  <input
                    type="checkbox"
                ...
                <td class="py-3 px-4 text-sm text-light-text-primary">
                  <span class="flex items-center">
                    <i class="ri-time-line mr-2 text-light-text-secondary"></i>
                    {{ formatDate(result.timestamp) }}
                  </span>
                </td>
                <td class="py-3 px-4 text-sm text-light-text-primary">
                  <span
                    class="px-2 py-1 rounded-md bg-blue-500/10 text-blue-600 border border-blue-500/20 inline-flex items-center"
                  >
                ...
                <td class="py-3 px-4">
                  <div class="flex gap-1 flex-nowrap">
                    <button
                      ...
                      class="action-button bg-blue-500/10 text-blue-600 border border-blue-500/30 hover:bg-blue-500/20"
                    >
                    ...
                    <button
                      ...
                      class="action-button bg-red-500/10 text-red-600 border border-red-500/30 hover:bg-red-500/20"
                    >
      ...
      <!-- 批量操作工具栏 -->
      <div
        class="flex items-center justify-between flex-wrap gap-4 p-4 rounded-xl border border-light-border bg-light-bg shadow-sm"
      >
        ...
        <div class="flex flex-wrap gap-3">
          <button
            ...
            :class="[
              !hasSelected
                ? 'bg-light-bg text-light-text-disabled border-light-border cursor-not-allowed'
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const isAllSelected = computed(() => {
      if (!props.subdomainScanResults?.length) return false;
      return selectedResults.value.length === props.subdomainScanResults.length;
    });
